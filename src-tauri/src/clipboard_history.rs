...
const DEFAULT_SHORTCUT: &str = "Ctrl+Q";
const LEGACY_DEFAULT_SHORTCUT: &str = "Ctrl+X";
const SHORTCUT_MIGRATION_FILE_NAME: &str = "clipboard-shortcut-ctrl-q-migrated";
...
pub fn init(app: &AppHandle) -> Result<(), String> {
    ...
    {
        let mut state = service.state.lock().map_err(|error| error.to_string())?;
        service.enforce_limit_locked(&mut state);
        service.persist_locked(&state)?;
    }
...
fn load_history(history_path: &Path) -> PersistedClipboardHistory {
    fs::read_to_string(history_path)
        ...
        .map(|mut history| {
            migrate_legacy_default_shortcut(history_path, &mut history.settings);
...
fn migrate_legacy_default_shortcut(history_path: &Path, settings: &mut ClipboardHistorySettings) {
    let migration_path = history_path.with_file_name(SHORTCUT_MIGRATION_FILE_NAME);
    ...
    if settings
        .shortcut
        .eq_ignore_ascii_case(LEGACY_DEFAULT_SHORTCUT)
    {
        settings.shortcut = DEFAULT_SHORTCUT.to_string();
    }

    fs::write(migration_path, b"migrated").ok();
}
...
impl ClipboardHistoryService {
    ...
    fn persist_locked(&self, state: &ClipboardHistoryState) -> Result<(), String> {
        ...
        let mut file = fs::File::create(&self.history_path)
            .map_err(|error| format!("Failed to create clipboard history file: {error}"))?;
        file.write_all(content.as_bytes())
            .map_err(|error| format!("Failed to write clipboard history: {error}"))?;
        file.sync_all()
            .map_err(|error| format!("Failed to flush clipboard history: {error}"))
    }
...
fn run_windows_clipboard_listener() -> Result<(), String> {
    unsafe {
        let class_name = w!("SurfisleClipboardHistoryListener");
        ...
        let hwnd = CreateWindowExW(
            WINDOW_EX_STYLE(0),
            class_name,
            w!("Surfisle Clipboard History Listener"),
            WINDOW_STYLE(0),
            0,
            0,
            0,
