param(
  ...
  [ValidateSet("codex", "claudeCode", "opencode")]
  [string]$Provider,
...
function Get-DefaultStatusPath {
  if ($env:SURFISLE_AGENT_STATUS_PATH) {
    return $env:SURFISLE_AGENT_STATUS_PATH
  }

  if ($env:APPDATA) {
    return Join-Path $env:APPDATA "com.ryancy.surfisle\agent-status.json"
  }

  return Join-Path $env:LOCALAPPDATA "com.ryancy.surfisle\agent-status.json"
}
...
function Get-AgentMarkerNames {
  ...
  if ($Provider -eq "opencode") {
    return @{
      Running = "agent-opencode-running.flag"
      Hold = "agent-opencode-running-hold.flag"
    }
  }
...
$mutex = New-Object System.Threading.Mutex($false, "Surfisle.AgentStatus")
$hasLock = $false

try {
  $hasLock = $mutex.WaitOne([TimeSpan]::FromSeconds(5))
  if (-not $hasLock) {
    throw "Timed out waiting for the Surfisle agent status file lock."
  }
  ...
  if (Test-Path -LiteralPath $StatusPath) {
    try {
      ...
      $state.opencode = Copy-AgentTaskStatus -Source $existing.opencode
    } catch {
      $state.codex = New-AgentTaskStatus
      $state.claudeCode = New-AgentTaskStatus
      $state.opencode = New-AgentTaskStatus
    }
  }
